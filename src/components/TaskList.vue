    ...
    <div class="text-center mt-5">
        ...
        <div class="text-start d-flex flex-column flex-md-row justify-content-between align-items-center mt-5">
            <div class="d-flex flex-column flex-md-row">
                ...
                <div>
                    <input
                        class="form-control rounded-3 p-2 px-3 mx-md-4 mx-0 my-3 my-md-0" type="date"
                        v-model="filterDate" @change="applyFilter"/>
                </div>
            </div>
            <div>
                <input class="form-control rounded-5 p-2 px-3" type="text" v-model="searchQuery" @input="applyFilter"
                       placeholder="Search"/>
            </div>
        </div>
        <div
        ...
        >
            <div class="d-flex flex-column text-start" :class="{'text-decoration-line-through':task.completed}">
                <div class="text-start d-flex align-items-center my-3 my-md-0">
                    <input class="form-check-input " type="checkbox" id="gridCheck"
                           @change="toggleEvent(task.id, !task.completed)" :checked="task.completed">
                    <div class="mx-2">
            ...
            <div class="flex-column flex-md-row ">
                <div>
                    <p class="text-warning text-center text-md-end">
                        {{ dayjs(task.created_at).tz('Asia/Tashkent').format('DD-MM-YYYY HH:mm:ss') }}</p>
                </div>
                <div class="d-flex flex-column flex-md-row">
                    <router-link
                        :to="'/updateTask/' + task.id"
                        class="btn btn-outline-success w-100 w-md-50 rounded-5 p-2 px-5 mx-0 mx-md-4 my-3 my-md-0">
                        Edit
                    </router-link>
                    <button @click="deleteTaskById(task.id)"
                            class="btn btn-outline-danger w-100 w-md-50 rounded-5 p-2 px-5">Delete
                    </button>
                </div>
            </div>
        </div>
